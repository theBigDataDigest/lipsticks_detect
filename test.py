...
##cover the color #D62352 to RGB_array##
...
def data_operate():
    ...
    #the third loop covert value_array to RGB_array#
      ...
      for i in range(sum):
          ...
          G=RGB_array[i][1]
          B=RGB_array[i][2]
          #RGB_temp[i]=abs((get[0]-R)+(get[1]-G)+(get[2]-B))
          #RGB_temp[i]=abs(get[0]*get[0]-R*R)*0.299+abs(get[1]*get[1]-G*G)*0.587+abs(get[2]*get[2]-B*B)*0.114
          RGB_temp[i]=abs(get[0]-R)*0.1+abs(get[1]*3/4-G)*0.7+abs(get[2]-B)*0.2
